...
class deeplabv3plus(nn.Module):
	def __init__(self, cfg):
		...
		self.threshold_cat_conv = nn.Sequential(
				nn.Conv2d(cfg.MODEL_ASPP_OUTDIM+cfg.MODEL_SHORTCUT_DIM, cfg.MODEL_ASPP_OUTDIM, 3, 1, padding=1,bias=True),
				SynchronizedBatchNorm2d(cfg.MODEL_ASPP_OUTDIM, momentum=cfg.TRAIN_BN_MOM),
				nn.ReLU(inplace=True),
				nn.Dropout(0.5),
		...
		self.threshold_sigmoid_conv = nn.Sequential(
		...
		self.threshold_sigmoid_conv.apply(self.init_weights)

		'''for m in self.modules():
			classname=m.__class__.__name__
			if classname.find('threshold') != -1:
				nn.init.normal_(m.weight, mean=0, std=0.000001)
	...
	def forward(self, x, feature_cat=None, phase='orig'):
		...
		threshold_result = self.threshold_cat_conv(feature_cat) 
		threshold_result = self.threshold_sigmoid_conv(threshold_result)
		threshold_result = self.upsample4(threshold_result)
	#	return result, threshold_result
		return feature_cat, result, threshold_result