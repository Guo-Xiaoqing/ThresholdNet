...
def train_one_batch(inputs_batched, labels_batched, net, seg_optimizer, thr_optimizer, list=None, phase='original'):
	...
	###############################################
	feature_batched, predicts_batched, threshold_batched = net(inputs_batched)
	...
	#	Simi_consistent_loss = cfg.weight_Similoss * MSE(sample_cosSim, list['cosSimilarity']).mean()
		...
		seg_loss = (loss + seg_jac_loss + MM_consistent_loss + Simi_consistent_loss)
	else:
...
#	threshold_gt = torch.clamp(threshold_gt, 0.0, 1.0)
	...
	if phase == 'mixup':
		...
		_, mixup_feature_predicts, mixup_feature_threshold = net(inputs_batched, feature_cat=mixup_deep_feature, phase=phase)
		...
		mixup_feature_threshold = mixup_feature_threshold.to(1) 
		Softmax_predicts_batched = nn.Softmax(dim=1)(mixup_feature_predicts)
		mixup_feature_mask = nn.Sigmoid()(50*(Softmax_predicts_batched-mixup_feature_threshold))
		mixup_feature_loss = criterion(mixup_feature_predicts, labels_batched)
		mixup_feature_jac_loss = Overlap_loss(labels_batched, mixup_feature_mask, eps=1e-7)
		mixup_feature_seg_loss = cfg.weight_MixupFeatureUpdate * (mixup_feature_loss+mixup_feature_jac_loss)
		...
		#### train threshold branch ####
		_, predicts_batched, threshold_batched = net(inputs_batched, feature_cat=mixup_deep_feature, phase=phase)
		predicts_batched = predicts_batched.to(1)  
		threshold_batched = threshold_batched.to(1) 
		Softmax_predicts_batched = nn.Softmax(dim=1)(predicts_batched)
		mask = nn.Sigmoid()(50*(Softmax_predicts_batched-threshold_batched))
		dice_loss1 = Overlap_loss(labels_batched, mask, eps=1e-7)
		margin = cfg.Threshold_margin
		gt_1_hot = make_one_hot(labels_batched)
		threshold_gt = torch.where(gt_1_hot == 1, Softmax_predicts_batched-margin, Softmax_predicts_batched+margin)
	#	threshold_gt = torch.clamp(threshold_gt, 0.0, 1.0)
		sup_loss1 = SoftCrossEntropy(threshold_batched, threshold_gt)
		input_edge_aware = torch.exp(-grad_criterion(inputs_batched)).to(1)
		grad_loss1 = grad_criterion(threshold_batched).mul(input_edge_aware).mean()
		thres_loss = cfg.weight_MixupFeatureUpdate*(dice_loss1+sup_loss1+grad_loss1)
	...
	if phase == 'mixup':
		return loss, seg_jac_loss, mixup_feature_seg_loss, dice_loss, sup_loss
	else:
		return loss, seg_jac_loss, dice_loss, grad_loss, sup_loss, sample_cosSim_cpu, feature
...
#	print(i)
	...
	for epoch in range(cfg.TRAIN_MINEPOCH, cfg.TRAIN_EPOCHS):			
		...
		for i_batch, (sample_batched, mixup_batched) in enumerate(zip(dataloader, dataloader_mixup)):
			...
			loss, seg_jac_loss, dice_loss, grad_loss, sup_loss, cosSim1, deep_feature1 = train_one_batch(inputs_batched1, labels_batched1, net, seg_optimizer, thr_optimizer)
			...
			if epoch >= cfg.Mixup_start_epoch:
				...
				deep_feature2, predicts_batched2, threshold_batched2 = net(inputs_batched2)
				...
				loss, seg_jac_loss, mixup_feature_seg_loss, dice_loss, sup_loss = train_one_batch(mixup_input, mixup_label, net, seg_optimizer, thr_optimizer, list=mixup_list, phase='mixup')

				mixup_running_loss += loss.item()
				mixup_seg_jac_running_loss += seg_jac_loss.item()
				mixup_dice_running_loss += dice_loss.item()
				mixup_grad_running_loss += 10*mixup_feature_seg_loss.item()
				mixup_average_running_loss += sup_loss.item()	
		...
		if epoch >= cfg.Mixup_start_epoch:
			print('Mixup:\tSegCE loss:%g \tSegJaccard loss:%g \tThrJaccard loss:%g \tSegMixup loss:%g \tThrSup loss:%g \n' %  (mixup_running_loss/i_batch, 
